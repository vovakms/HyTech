...
int hcsqlinit(){
    ...
    //Вытаскиваем функции из hscli.dll
    ...
    hcSqlAllocStmt = (HCSQLGETCONNECTATTR)GetProcAddress(hinstLib, "hcSqlAllocStmt"); /* Создать оператор */
    ...
    hcSqlNumResultCols = (HCSQLGETSTMTATTR)GetProcAddress(hinstLib, "hcSqlNumResultCols"); /* возвращает количество колонок в результате. */
